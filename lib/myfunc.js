...
exports.smsg = (client, m, store) => {
    ...
    if (m.message) {
        ...
        m.body = m.message.conversation || m.msg.caption || m.msg.text || (m.mtype == 'listResponseMessage') && m.msg.singleSelectReply.selectedRowId || (m.mtype == 'templateButtonReplyMessage') && m.msg.selectedId || (m.mtype == 'messageContextInfo') && m.msg.selectedButtonId || m.text;
    ...
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? client.sendMessage(chatId, { image: text, ...options }, { quoted: m }) : client.sendMessage(chatId, { text: text, ...options }, { quoted: m });
